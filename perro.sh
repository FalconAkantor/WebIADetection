...
# Archivo de bloqueo
LOCKFILE="$OUTPUT_DIR/perro.lock"
...
LOG_FILE="$OUTPUT_DIR/perros.log"
...
python3 "$OUTPUT_DIR/detect.py" --source "$OUTPUT_FILE" --weights "$OUTPUT_DIR/yolov5s.pt" --conf-thres 0.35 --class 16
...
# Enviar mensaje con el total de fotogramas con perros detectadas
echo "Analizando detección de perros..."
detections=0
detection_frames=()
...
# Enviar resumen con el total de fotogramas con perros detectadas
if [ $detections -eq 0 ]; then
    send_message "No se detectó a ningun perro en el video."
else
    send_message "Se detectaron un total de $detections fotogramas con perros en el video."
    echo "Extrayendo fotogramas del video procesado donde se detectó a un perro..."
