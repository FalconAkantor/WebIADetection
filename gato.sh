...
python3 "$OUTPUT_DIR/detect.py" --source "$OUTPUT_FILE" --weights "$OUTPUT_DIR/yolov5s.pt" --conf-thres 0.35 --class 15
...
# Enviar mensaje con el total de fotogramas con gatos detectadas
echo "Analizando detección de gatos..."
detections=0
detection_frames=()
...
# Enviar resumen con el total de fotogramas con gatos detectadas
if [ $detections -eq 0 ]; then
    send_message "No se detectó a ningun gato en el video."
else
    send_message "Se detectaron un total de $detections fotogramas con gatos en el video."
    echo "Extrayendo fotogramas del video procesado donde se detectó a un gato..."
